...
class RestoreShoppingNoteScreen extends StatefulWidget {
  ...
  const RestoreShoppingNoteScreen({Key? key, required this.id, required this.title, required this.date,required this.capital,required this.remain ,this.itemTitles, this.prices, required this.totalCost}) : super(key: key);

  @override
  State<RestoreShoppingNoteScreen> createState() => _RestoreShoppingNoteScreenState();
}
              ...
              appBar: AppBar(
                ...
                actions: [
                         GestureDetector(onTap: ()async{
                          final response = await _fetchShoppingDataController.deleteRecycleBinData(id: widget.id!);
                          if(response == true){
                  ...
                  GestureDetector(onTap: () async{
                    ...
                    final response = await _fetchShoppingDataController.restoreDeletedData(
                      id: widget.id,
                      time: _time.trim(),
                          ...
                          children: [
                            ...
                            Expanded(
                              child: Card(
                                ...
                                child: Padding(
                                  padding: const EdgeInsets.all(10.0),
                                  child: Text('${totalCost.toStringAsFixed(2)} tk',
                                    maxLines: 1,
                                    overflow: TextOverflow.ellipsis,
                                    style: const TextStyle(
                                        fontSize: 18,
